...
LogicalResult triton::gpu::specializeLoadMMADependencies(scf::ForOp &loop,
  ...
  auto isLoadPipelineable = [&](Operation *op) {
    SmallVector<Operation*> loadOps = {aChain.back(), bChain.back()};
    if (!aScaleChain.empty())
      loadOps.push_back(aScaleChain.back());
    if (!bScaleChain.empty())
      loadOps.push_back(bScaleChain.back());
    return llvm::is_contained(loadOps, op);
  };
  ...
  auto allocate = [&](const SmallVector<Operation *> &chain) -> std::tuple<Operation *, RankedTensorType, SharedEncodingTrait, Value> {
    if (chain.empty())
  ...
  if (aScaleLoad) loadSizeInBytes += product(aScaleType.getShape()) * aScaleType.getElementTypeBitWidth() / 8;
  if (bScaleLoad) loadSizeInBytes += product(bScaleType.getShape()) * bScaleType.getElementTypeBitWidth() / 8;

  // Insert before the group of loads.
  SmallVector<Operation*> allLoads{aLoad, bLoad};
  if (aScaleLoad) allLoads.push_back(aScaleLoad);
  if (bScaleLoad) allLoads.push_back(bScaleLoad);
  ...
  auto lowerLoadAndPropagate = [&](Operation *load, Value alloc, Value barrier) {
  ...
  if (aScaleLoad) lowerLoadAndPropagate(aScaleLoad, aScaleAlloc, curLoadBar);
  if (bScaleLoad) lowerLoadAndPropagate(bScaleLoad, bScaleAlloc, curLoadBar);
  ...
  auto allUsers = llvm::to_vector(llvm::concat<Operation *>(aChain, bChain, aScaleChain, bScaleChain));
